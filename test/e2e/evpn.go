...
func setupMACVRFAgnhost(ictx infraapi.Context, containerName, networkName, bridgeName string, vid int, ipFamilies sets.Set[utilnet.IPFamily], subnets []string, reassignIPs bool) error {
	...
	var info *evpnAgnhostInfo
	if reassignIPs {
		info, err = createEVPNAgnhost(ictx, networkName, containerName, ipFamilies, nil, "", "")
	} else {
	...
	if reassignIPs {
		...
		if ipFamilies.Has(utilnet.IPv6) {
			sysctlPath := fmt.Sprintf("net.ipv6.conf.%s.disable_ipv6", info.agnhostInterface)
			if _, err := infraprovider.Get().ExecExternalContainerCommand(container,
				[]string{"sysctl", "-w", sysctlPath + "=0"}); err != nil {
				return fmt.Errorf("failed to enable IPv6 on %s/%s: %w", containerName, info.agnhostInterface, err)
			}
		}
		...
		framework.Logf("Reassigned %s IPs to CUDN IPs: %v", containerName, agnhostIPs)
	}
...
func runEVPNNetworkAndServers(
	...
	if hasMACVRF {
		...
		err = setupMACVRFAgnhost(ictx, macVRFAgnhostName, macVRFNetworkName, bridgeName, macVRFVID, ipFamilySet, cudnSubnetsFromSpec, reassign)
		if err != nil {
			return err
		}
	}
