			...
			for fam, podAIP := range podAIPs {
				...
				go func() {
					...
					supOutB, supErrB = e2ekubectl.RunKubectl(
						bumTestNamespace.Name, "exec", tcpdumpPodRemote.Name, "--", "sh", "-c",
						fmt.Sprintf("timeout -s KILL 25 tcpdump -i %s -nne -c 1 %s 2>&1", l2SVI, bumE2EQuoteTcpdumpFilter(bumSupComboFilter)))
				}()
				...
				gomega.Expect(supErrB).To(gomega.HaveOccurred(),
					fmt.Sprintf("node %s: received broadcast ARP/NS for PodA %s or external %s on %s unexpectedly", nodeB, podAIP, extMACVRFIPs[fam], l2SVI))
