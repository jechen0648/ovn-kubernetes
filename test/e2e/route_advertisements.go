...
var _ = ginkgo.Describe("BGP: When default podNetwork is advertised", feature.RouteAdvertisements, func() {
	...
	var frrPeerIPv4, frrPeerIPv6 string // FRR container IPs on the BGP peer network (used for route checks)
	var nodePeerIPsV4 map[string]string // Node name -> IPv4 peer network IP (for route checks)
	var nodePeerIPsV6 map[string]string // Node name -> IPv6 peer network IP (for route checks)
	...
	ginkgo.BeforeEach(func() {
		...
		ginkgo.By("Running a BGP network with an agnhost server for default network")
		...
		// This ensures OVN can properly handle return traffic
		...
		// For default network, we use the PRIMARY network for BGP peering
		// This aligns with OVN's gateway configuration and ensures traffic flows correctly
		frrPeerIPv4, frrPeerIPv6 = frrContainerIPv4, frrContainerIPv6
		framework.Logf("Using FRR primary network IPv4 address: %s and/or IPv6: %s for BGP peering", frrPeerIPv4, frrPeerIPv6)
		...
		if frrPeerIPv4 != "" {
			frrContainerIPs = append(frrContainerIPs, frrPeerIPv4)
		}
		if frrPeerIPv6 != "" {
			frrContainerIPs = append(frrContainerIPs, frrPeerIPv6)
		}
		...
		// For default network, node IPs for route verification are the nodes' primary network IPs
		// (these are the Kubernetes InternalIPs)
		// Note: nodePeerIPsV4 is used for IPv4 route verification; IPv6 typically uses link-local addresses
		// but we collect nodePeerIPsV6 for consistency and potential future use
		ginkgo.By("Getting node primary network IPs for route verification")
		nodePeerIPsV4 = make(map[string]string)
		nodePeerIPsV6 = make(map[string]string)
		...
		for _, node := range nodeList.Items {
			...
			// Store IPv4 address if cluster supports IPv4 and we found one
			if isIPv4Supported(f.ClientSet) && nodeIPv4 != "" {
				nodePeerIPsV4[node.Name] = nodeIPv4
				framework.Logf("Node %s primary network IPv4: %s", node.Name, nodeIPv4)
			}
			// Store IPv6 address if cluster supports IPv6 and we found one
			if isIPv6Supported(f.ClientSet) && nodeIPv6 != "" {
				nodePeerIPsV6[node.Name] = nodeIPv6
		...
		ginkgo.It("tests are run towards the external agnhost echo server", func() {
			...
			for _, node := range nodes.Items {
				if isIPv4Supported(f.ClientSet) {
					...
					framework.Logf("Checking for server's route in node %s (expecting next hop %s)", node.Name, frrPeerIPv4)
					gomega.Eventually(func() bool {
						...
						return strings.Contains(routes, frrPeerIPv4)
				...
				if isIPv6Supported(f.ClientSet) {
					...
					gomega.Eventually(func() bool {
						...
						// The exact link-local depends on which interface FRR uses for the primary network
			...
			// sample (routes via peer network IPs, not Kubernetes internal IPs)
			...
			for _, serverContainerIP := range serverContainerIPs {
				for _, node := range nodes.Items {
					...
					nodeIPOverrides := nodePeerIPsV4
					if utilnet.IsIPv6String(serverContainerIP) {
						nodeIPOverrides = nodePeerIPsV6
					}
...
var _ = ginkgo.Describe("BGP: Pod to external server when CUDN network is advertised", feature.RouteAdvertisements, func() {
	...
	ginkgo.DescribeTable("Route Advertisements",
		func(cudnTemplate *udnv1.ClusterUserDefinedNetwork, ra *rav1.RouteAdvertisements) {
			...
			gomega.Eventually(clusterUserDefinedNetworkReadyFunc(f.DynamicClient, cUDN.Name), 60*time.Second, time.Second).Should(gomega.Succeed())
